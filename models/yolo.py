    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.nkpt is None or self.nkpt==0:
                ...
                # yf: edit
                x_det = self.m[i](x[i])
                bs, _, ny, nx = x_det.shape
                x_det = x_det.view(bs, self.na, self.no_det, ny, nx)
                x_kpt = self.m_kpt[i](x[i])
                x_kpt = x_kpt.view(bs, self.na, self.no_kpt, ny, nx)
                x[i] = torch.cat((x_det, x_kpt), axis=2).view(bs, -1, ny, nx)
