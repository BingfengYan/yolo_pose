...
ILayer* convKPTBlock(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int out_kpt, std::string lname){
    // printf("%s\n");
    auto gconv0 = convBlock(network, weightMap, input, outch, ksize, 1, outch, lname+".0");
    auto conv1 = convBlock(network, weightMap,  *gconv0->getOutput(0), outch, 1, 1, 1, lname+".1");

    auto gconv2 = convBlock(network, weightMap, *conv1->getOutput(0), outch, ksize, 1, outch, lname+".2");
    auto conv3 = convBlock(network, weightMap,  *gconv2->getOutput(0), outch, 1, 1, 1, lname+".3");

    auto gconv4 = convBlock(network, weightMap, *conv3->getOutput(0), outch, ksize, 1, outch, lname+".4");
    auto conv5 = convBlock(network, weightMap,  *gconv4->getOutput(0), outch, 1, 1, 1, lname+".5");

    auto gconv6 = convBlock(network, weightMap, *conv5->getOutput(0), outch, ksize, 1, outch, lname+".6");
    auto conv7 = convBlock(network, weightMap,  *gconv6->getOutput(0), outch, 1, 1, 1, lname+".7");

    auto gconv8 = convBlock(network, weightMap, *conv7->getOutput(0), outch, ksize, 1, outch, lname+".8");
    auto conv9 = convBlock(network, weightMap,  *gconv8->getOutput(0), outch, 1, 1, 1, lname+".9");

    auto gconv10 = convBlock(network, weightMap, *conv9->getOutput(0), outch, ksize, 1, outch, lname+".10");
    auto conv11 = network->addConvolutionNd(*gconv10->getOutput(0), out_kpt, DimsHW{ 1, 1 }, weightMap[lname+".11.weight"], weightMap[lname+".11.bias"]);
...
ILayer* focus_contract(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int inch, int outch, int ksize, std::string lname) {
    ISliceLayer *s1 = network->addSlice(input, Dims3{ 0, 0, 0 }, Dims3{ inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2 }, Dims3{ 1, 2, 2 });
    ISliceLayer *s2 = network->addSlice(input, Dims3{ 0, 1, 0 }, Dims3{ inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2 }, Dims3{ 1, 2, 2 });
    ISliceLayer *s3 = network->addSlice(input, Dims3{ 0, 0, 1 }, Dims3{ inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2 }, Dims3{ 1, 2, 2 });
    ISliceLayer *s4 = network->addSlice(input, Dims3{ 0, 1, 1 }, Dims3{ inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2 }, Dims3{ 1, 2, 2 });
    ITensor* inputTensors[] = { s1->getOutput(0), s3->getOutput(0), s2->getOutput(0), s4->getOutput(0) };
    auto cat = network->addConcatenation(inputTensors, 4);
    auto conv = convBlock(network, weightMap, *cat->getOutput(0), outch, ksize, 1, 1, lname + ".conv");
    return conv;
}
...
// ILayer* C3Temp(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int c1, int c2, int n, bool shortcut, int g, float e, std::string lname) {
//     int c_ = (int)((float)c2 * e);
//     auto cv1 = convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv1");
//     auto cv2 = convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv2");
//     // auto b = bottleneck(network, weightMap, *cv1->getOutput(0), 256, 256, true, 1, 1.0, "model.11.m.0");

//     auto mcv1 = convBlock(network, weightMap, *cv1->getOutput(0), 256, 1, 1, 1,  "model.11.m.0.cv1");
//     // ITensor *y1 = cv1->getOutput(0);
//     // for (int i = 0; i < n; i++) {
//     //     return cv1;
//     //     auto b = bottleneck(network, weightMap, *y1, c_, c_, shortcut, g, 1.0, lname + ".m." + std::to_string(i));
...
IPluginV2Layer* addYoLoLayer(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, std::string lname, std::vector<ILayer*> dets) {
