...
#define BATCH_SIZE Yolo::MAX_BATCH
...
const char* OUTPUT1_BLOB_NAME = "boxes";
const char* OUTPUT2_BLOB_NAME = "classes";
const char* OUTPUT3_BLOB_NAME = "points";
...
ICudaEngine* build_engine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    ...
    auto yolo = addYoLoLayer(network, weightMap, "model.24", std::vector<ILayer*>{det0, det1, det2});
    yolo->getOutput(0)->setName(OUTPUT_BLOB_NAME);
    network->markOutput(*yolo->getOutput(0));
    yolo->getOutput(1)->setName(OUTPUT1_BLOB_NAME);
    network->markOutput(*yolo->getOutput(1));
    yolo->getOutput(2)->setName(OUTPUT2_BLOB_NAME);
    network->markOutput(*yolo->getOutput(2));
    yolo->getOutput(3)->setName(OUTPUT3_BLOB_NAME);
    network->markOutput(*yolo->getOutput(3));
    // Build engine
...
ICudaEngine* build_engine_p6(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    ...
    /* ------ yolov5 backbone------ */
    // auto conv0 = convBlock(network, weightMap, *data,  get_width(64, gw), 6, 2, 1,  "model.0");
    auto conv0 = focus_contract(network, weightMap, *data,  3, get_width(64, gw), 3,  "model.0");
    ...
    auto sppf10 = SPPF(network, weightMap, *conv9->getOutput(0), get_width(1024, gw), get_width(1024, gw), 3, "model.10");
    auto c3_11 = C3(network, weightMap, *sppf10->getOutput(0), get_width(1024, gw), get_width(1024, gw), get_depth(3, gd), false, 1, 0.5, "model.11");
    ...
    auto* kpt0 = convKPTBlock(network, weightMap, *c3_23->getOutput(0), get_width(256, gw), 3, 3*Yolo::NO_KPT, "model.33.m_kpt.0");
    auto* kpt1 = convKPTBlock(network, weightMap, *c3_26->getOutput(0), get_width(512, gw), 3, 3*Yolo::NO_KPT, "model.33.m_kpt.1");
    auto* kpt2 = convKPTBlock(network, weightMap, *c3_29->getOutput(0), get_width(768, gw), 3, 3*Yolo::NO_KPT, "model.33.m_kpt.2");
    auto* kpt3 = convKPTBlock(network, weightMap, *c3_32->getOutput(0), get_width(1024, gw), 3, 3*Yolo::NO_KPT, "model.33.m_kpt.3");

    ITensor* det_kpt0[] = {det0->getOutput(0), kpt0->getOutput(0)};
    ILayer* det_kpt0_cat = network->addConcatenation(det_kpt0, 2);
    ITensor* det_kpt1[] = {det1->getOutput(0), kpt1->getOutput(0)};
    ILayer* det_kpt1_cat = network->addConcatenation(det_kpt1, 2);
    ITensor* det_kpt2[] = {det2->getOutput(0), kpt2->getOutput(0)};
    ILayer* det_kpt2_cat = network->addConcatenation(det_kpt2, 2);
    ITensor* det_kpt3[] = {det3->getOutput(0), kpt3->getOutput(0)};
    ILayer* det_kpt3_cat = network->addConcatenation(det_kpt3, 2);

    auto yolo = addYoLoLayer(network, weightMap, "model.33", std::vector<ILayer*>{det_kpt0_cat, det_kpt1_cat, det_kpt2_cat, det_kpt3_cat});
    yolo->getOutput(0)->setName(OUTPUT_BLOB_NAME);
    network->markOutput(*yolo->getOutput(0));
    yolo->getOutput(1)->setName(OUTPUT1_BLOB_NAME);
    network->markOutput(*yolo->getOutput(1));
