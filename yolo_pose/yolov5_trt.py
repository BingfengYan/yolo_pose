...
def plot_one_box(x, img, points=None, color=None, label=None, line_thickness=None):
    """
    ...
    for p in points.reshape(-1, 2):
        cv2.circle(img, (int(p[0]), int(p[1])), tl, color, 2)
        # cv2.putText(img, str(p[2])[:3], (int(p[0]), int(p[1])), 0, tl / 5, color)
    if label:
...
class YoLov5TRT(object):
    ...
    def infer(self, raw_image_generator):
        ...
        batch_origin_h = []
        batch_origin_w = []
        images = list(raw_image_generator)
        batch_size = len(images)
        batch_input_image = np.empty(shape=[self.batch_size, 3, self.input_h, self.input_w], dtype=np.float32)
        ...
        # batch_input_image[...] = np.loadtxt('input.txt').reshape(1, 3, 640, 640).astype(np.float32)
        # img = (batch_input_image.transpose(0, 2, 3, 1) * 255).astype(np.uint8)[0][..., ::-1]
        # cv2.imwrite('tmp.png', img)
        # batch_input_image = np.zeros(shape=[batch_size, 3, self.input_h, self.input_w], dtype=np.float32)
        ...
        output_prob = host_outputs[0].reshape(self.batch_size, MAX_NUM_DET, -1)
    ...
    def xyxy2xyxy(self, origin_h, origin_w, x):
        """
        description:    Convert nx4 boxes from [x, y, w, h] to [x1, y1, x2, y2] where xy1=top-left, xy2=bottom-right
        param:
            origin_h:   height of original image
    ...
    def xy2xy(self, origin_h, origin_w, x, stride=2):
        y = x.copy()
        r_w = self.input_w / origin_w
        r_h = self.input_h / origin_h
        if r_h > r_w:
            y[..., 0::stride] = x[..., 0::stride]
            y[..., 1::stride] = x[..., 1::stride] - (self.input_h - r_w * origin_h) / 2
            y[..., 0::stride] /= r_w
            y[..., 1::stride] /= r_w
        else:
            y[..., 0::stride] = x[..., 0::stride] - (self.input_w - r_h * origin_w) / 2
            y[..., 1::stride] = x[..., 1::stride]
            y[..., 0::stride] /= r_h
            y[..., 1::stride] /= r_h
    ...
    def post_process(self, output, origin_h, origin_w):
        ...
        """
        boxes = output[output[:, 4] >= CONF_THRESH]
        boxes[:, :4] = self.xyxy2xyxy(origin_h, origin_w, boxes[:, :4])
        boxes[:, 6:] = self.xy2xy(origin_h, origin_w, boxes[:, 6:], 2)
    ...
    def non_max_suppression(self, prediction, origin_h, origin_w, conf_thres=0.5, nms_thres=0.4):
        ...
        boxes[:, 6:] = self.xy2xy(origin_h, origin_w, boxes[:, 6:], 3)
